 ...
 */
class RepositoryItemsTable extends WP_List_Table
{
    ...
    {
        global $wpdb;
        $table_name = $wpdb->prefix . 'nada_referentiel_items';
    ...
    private function buildWhereClause($search_term)
    {
        ...
        // Filter by referentiel ID if provided
        $referentiel_id = isset($_REQUEST['ref_id']) ? absint($_REQUEST['ref_id']) : 0;
        if ($referentiel_id > 0) {
            $where .= $wpdb->prepare(" AND referentiel_id = %d", $referentiel_id);
        }
